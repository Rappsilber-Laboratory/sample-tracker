#!/usr/bin/env python3
"""Bulk-create one experiment + samples for project BNC ("Blank and cleaning")
and link all 18 BNC files.

BNC's purpose is to track blank / column-cleaning / corrupted runs, so the
"weird" batch_names ARE the data. All 18 files have distinct batch_names
(including one empty string and one trailing `_CORRUPTED`).

Modelled as **one experiment** (`BNC001`, "BNC 001"), **one sample per
distinct batch_name**. The empty batch_name maps to `sample.code = 'unnamed'`
(the file otherwise carries no biological identity).

Refuses to run if BNC already has any experiments or any BNC file already
has sample_id set.
"""
import sqlite3
import sys
from collections import OrderedDict
from pathlib import Path

DB_PATH = Path(__file__).resolve().parent.parent / "samples.db"
PROJECT_CODE = "BNC"
EXPERIMENT_CODE = "BNC001"
EXPERIMENT_NAME = "BNC 001"
EMPTY_PLACEHOLDER = "unnamed"


def main():
    if not DB_PATH.exists():
        sys.exit(f"{DB_PATH} not found")

    conn = sqlite3.connect(DB_PATH)
    conn.execute("PRAGMA foreign_keys = ON")
    try:
        with conn:  # transaction
            project = conn.execute(
                "SELECT id FROM project WHERE code = ?", (PROJECT_CODE,)
            ).fetchall()
            if len(project) != 1:
                sys.exit(
                    f"expected exactly 1 project with code={PROJECT_CODE}, "
                    f"found {len(project)}"
                )
            project_id = project[0][0]

            existing_exps = conn.execute(
                "SELECT COUNT(*) FROM experiment WHERE project_id = ?",
                (project_id,),
            ).fetchone()[0]
            if existing_exps:
                sys.exit(
                    f"project {PROJECT_CODE} already has {existing_exps} "
                    "experiments. Refusing to populate."
                )
            already_linked = conn.execute(
                "SELECT COUNT(*) FROM file "
                "WHERE project_code = ? AND sample_id IS NOT NULL",
                (PROJECT_CODE,),
            ).fetchone()[0]
            if already_linked:
                sys.exit(
                    f"{already_linked} {PROJECT_CODE} files already have a "
                    "sample_id set. Refusing to populate."
                )

            files = conn.execute(
                "SELECT id, batch_name FROM file WHERE project_code = ? "
                "ORDER BY id",
                (PROJECT_CODE,),
            ).fetchall()
            if not files:
                sys.exit(f"no files found for project_code={PROJECT_CODE}")

            cur = conn.execute(
                "INSERT INTO experiment (project_id, code, name) "
                "VALUES (?, ?, ?)",
                (project_id, EXPERIMENT_CODE, EXPERIMENT_NAME),
            )
            exp_id = cur.lastrowid

            sample_ids = OrderedDict()
            for fid, bn in files:
                code = bn if bn else EMPTY_PLACEHOLDER
                if code not in sample_ids:
                    cur = conn.execute(
                        "INSERT INTO sample "
                        "(experiment_id, name, code, crosslinked_sample) "
                        "VALUES (?, ?, ?, 0)",
                        (exp_id, code.replace("_", " "), code),
                    )
                    sample_ids[code] = cur.lastrowid

            link_count = 0
            for fid, bn in files:
                code = bn if bn else EMPTY_PLACEHOLDER
                conn.execute(
                    "UPDATE file SET sample_id = ? WHERE id = ?",
                    (sample_ids[code], fid),
                )
                link_count += 1

        violations = list(conn.execute("PRAGMA foreign_key_check"))
        if violations:
            print("FK violations:", violations, file=sys.stderr)
            sys.exit(2)

        print(
            f"Created 1 experiment, {len(sample_ids)} samples, "
            f"linked {link_count} files for project {PROJECT_CODE}."
        )
    finally:
        conn.close()


if __name__ == "__main__":
    main()
